				...
				<v-card>
					...
					<v-card-text>
						<v-form ref="formEditar">
							<v-text-field
								label="Nombre"
								v-model="clienteAEditar.nombre"
							></v-text-field>
							<v-text-field
								label="Dirección"
								v-model="clienteAEditar.direccion"
							></v-text-field>
							<v-text-field
								label="Email"
								v-model="clienteAEditar.email"
							></v-text-field>
							<v-text-field
								label="Teléfono"
								v-model="clienteAEditar.telefono"
							></v-text-field>
						</v-form>
					</v-card-text>
					<v-card-actions>
						<v-spacer></v-spacer>
						<v-btn color="red darken-1" text @click="dialogEditar = false"
							>Cancelar</v-btn
						>
	...
	methods: {
		...
		async guardarEdicion() {
			try {
				...
				const index = this.clientes.findIndex(
					(t) => t.id_cliente === this.clienteAEditar.id_cliente
				);
				if (index !== -1) {
